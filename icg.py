...
# ------------------ Phase 2.5: Parse Tree ------------------
def build_parse_tree(postfix):
    stack = []
    for token in postfix:
        if token not in "+-*/<>==!=":
            ...
            right = stack.pop()
            left = stack.pop()
            node = {'val': token, 'left': left, 'right': right}
            stack.append(node)
    return stack[0]

def write_parse_tree(node, indent=0, f=None):
    if node:
        f.write("  " * indent + str(node['val']) + "\n")
        if 'left' in node:
            write_parse_tree(node['left'], indent + 1, f)
        if 'right' in node:
            write_parse_tree(node['right'], indent + 1, f)
        ...
        if line.startswith("int") and '(' not in line:
            ...
            for item in vars_defs:
                if '=' in item:
                    ...
                    tree = build_parse_tree(postfix)
                    with open("parse_tree.txt", "a") as f:
                        f.write(f"Parse Tree for: {var} = {expr}\n")
                        write_parse_tree(tree, 1, f)
                        f.write("\n")
            ...
            tree = build_parse_tree(postfix)
            with open("parse_tree.txt", "a") as f:
                f.write(f"Parse Tree for WHILE: {condition}\n")
                write_parse_tree(tree, 1, f)
                f.write("\n")
            tac, result = generate_TAC(postfix)
            tac_code.extend(tac)
